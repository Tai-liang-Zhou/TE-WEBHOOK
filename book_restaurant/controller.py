    ...
    def post(self):
        ...
        te_payload = {
            ...
            "task_info": {
                ...
                "chair_num": task_info.get('seat_num_children', None),
                "lastname": task_info.get('lastname', None),
                "phone": task_info.get('phone', None),
                "note": task_info.get('note', None),
            }
        }
        # LOG.debug('data received from TE: %s' % json.dumps(te_payload, ensure_ascii=False, indent=4))
        ...
        payload = {
            ...
            "task_info": {
                ...
                "kid_num": int(task_info['kid_num']) if task_info['kid_num'] is not None else 0,
                "chair_num": int(task_info['chair_num']) if task_info['chair_num'] is not None else 0,
        ...
        # Assign 0 to chair_num if user doesn't want to baby chair
        update_kv_map['chair_num'] = int(task_info['chair_num']) if task_info['chair_num'] is not None else 0
...
class ConvertParams(Resource):
    def post(self):
        ...
        # parse exact date
        dt = get_datetime(exact_date)
        exact_hour = get_hourtime(exact_hour)
        exact_minute = int(get_num(exact_minute))
...
class ResetParams(Resource):
    def post(self):
        ...
        # print the format of json
        LOG.debug('data received from TE: %s' % json.dumps(json_from_request, ensure_ascii=False, indent=4))
